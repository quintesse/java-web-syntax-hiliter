...
public class SQLTokenMarker extends TokenMarker {
	private int offset, lastOffset, lastKeyword, lineLength;
	...
	public byte markTokensImpl(byte token, Segment line, int lineIndex) {
		...
		lineLength = line.count + offset;

		loop: for (int i = offset; i < lineLength; i++) {
			switch (line.array[i]) {
			case '*':
				if (token == Token.COMMENT1 && lineLength - i >= 1 && line.array[i + 1] == '/') {
					token = Token.NULL;
					i++;
					addToken(line, lastOffset, (i + 1) - lastOffset, Token.COMMENT1);
					lastOffset = i + 1;
				} else if (token == Token.NULL) {
					searchBack(line, i);
					addToken(line, i, 1, Token.OPERATOR);
					lastOffset = i + 1;
				}
				break;
			case '[':
			...
			case ']':
				if (token == Token.LITERAL1 && literalChar == '[') {
					token = Token.NULL;
					literalChar = 0;
					addToken(line, lastOffset, (i + 1) - lastOffset, Token.LITERAL1);
					lastOffset = i + 1;
				}
				break;
			case '.':
			...
			case ':':
				if (token == Token.NULL) {
					addToken(line, lastOffset, (i + 1) - lastOffset, Token.LABEL);
					lastOffset = i + 1;
				}
				break;
			case '/':
				if (token == Token.NULL) {
					if (lineLength - i >= 2 && line.array[i + 1] == '*') {
						searchBack(line, i);
			...
			case '-':
				...
				break;
			case '!':
				if (isTSQL && token == Token.NULL && lineLength - i >= 2
		...
		if (token == Token.NULL)
			searchBack(line, lineLength, false);
	...
	private void searchBack(Segment line, int pos, boolean padNull) {
		...
		if (padNull && lastOffset < pos)
			addToken(line, lastOffset, pos - lastOffset, Token.NULL);
	}
}
