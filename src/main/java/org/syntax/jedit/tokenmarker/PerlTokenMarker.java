...
public class PerlTokenMarker extends TokenMarker {
	...
	public byte markTokensImpl(byte _token, Segment line, int lineIndex) {
		...
		if (token == Token.LITERAL1 && lineIndex != 0 && lineInfo[lineIndex - 1].obj != null) {
			...
			if (str != null && str.length() == line.count && SyntaxUtilities.regionMatches(false, line, offset, str)) {
				addToken(line, offset, line.count, token);
				token = Token.NULL;
			} else {
				addToken(line, offset, line.count, token);
		...
		loop: for (int i = offset; i < lineLength; i++) {
			...
			case Token.NULL:
				...
				case '=':
					...
					if (i == offset) {
						...
						addToken(line, lastOffset, lineLength - i, token);
						lastOffset = lastKeyword = lineLength;
						break loop;
					}
					doKeyword(line, i, c);
				...
				case '<':
					...
					if (backslash)
						...
						if (lineLength - i > 2 && array[i1] == '<' && !Character.isWhitespace(array[i + 2])) {
							addToken(line, lastOffset, i - lastOffset, token);
							lastOffset = lastKeyword = i;
							token = Token.LITERAL1;
							int len = lineLength - (i + 2);
							if (array[lineLength - 1] == ';')
								len--;
				...
				case ':':
					...
					if (lastKeyword != 0)
						break;
					addToken(line, lastOffset, i1 - lastOffset, Token.LABEL);
				...
				case '-':
					...
					if (doKeyword(line, i, c))
						break;
					if (i != lastKeyword || lineLength - i <= 1)
						break;
					switch (array[i1]) {
					case 'r':
					case 'w':
					...
					case 'C':
						addToken(line, lastOffset, i - lastOffset, token);
						addToken(line, i, 2, Token.KEYWORD3);
				...
				if (backslash)
					...
					if (matchChar == '\0') {
						...
						if (token == S_TWO) {
							...
							token = S_END;
							addToken(line, lastOffset, i1 - lastOffset, Token.LITERAL2);
							lastOffset = lastKeyword = i1;
						}
					}
				}
			...
			case Token.COMMENT2:
				...
				if (i == offset) {
					addToken(line, offset, line.count, token);
					if (lineLength - i > 3 && SyntaxUtilities.regionMatches(false, line, offset, "=cut"))
						token = Token.NULL;
					lastOffset = lastKeyword = lineLength;
					break loop;
				}
		...
		case Token.LITERAL2:
			...
			break;
		case S_END:
			addToken(line, lastOffset, lineLength - lastOffset, Token.LITERAL2);
			token = Token.NULL;
			break;
		case S_ONE:
		case S_TWO:
			addToken(line, lastOffset, lineLength - lastOffset, Token.INVALID); // XXX
			token = Token.NULL;
			break;
		default:
			addToken(line, lastOffset, lineLength - lastOffset, token);
	...
	private boolean doKeyword(Segment line, int i, char c) {
		...
		if (token == S_END) {
			addToken(line, lastOffset, i - lastOffset, Token.LITERAL2);
			token = Token.NULL;
			lastOffset = i;
			lastKeyword = i1;
			return false;
		...
		if (id == S_ONE || id == S_TWO) {
			...
			addToken(line, lastKeyword, len, Token.LITERAL2);
			lastOffset = i;
			lastKeyword = i1;
