...
public abstract class TokenMarker {
	...
	public Token markTokens(Segment line, int lineIndex) {
		...
		addToken(null, 0, 0, Token.END);

		return firstToken;
	}

	/**
	 * An abstract method that splits a line up into tokens. It should parse
	 * the line, and call <code>addToken()</code> to add syntax tokens to
	 * the token list. Then, it should return the initial token type for the
	 * next line.
	 * <p>
	 * 
	 * For example if the current line contains the start of a multiline
	 ...
	 * If this is true, the object using this token marker is required to
	 * pass all lines in the document to the <code>markTokens()</code>
	 * method (in turn).
	 * <p>
	 * 
	...
	/**
	 * Informs the token marker that line have been deleted from the
	 * document. This removes the lines in question from the
	...
	/**
	 * Returns true if the next line should be repainted. This will return
	 * true after a line has been tokenized that starts a multiline token
	...
	/**
	 * The number of lines in the model being tokenized. This can be less
	 * than the length of the <code>lineInfo</code> array.
	 */
	 ...
	 * specified index. This enlarges it if necessary. No action is taken if
	 * the array is large enough already.
	 * <p>
	 * 
	 * It should be unnecessary to call this under normal circumstances;
	 * <code>insertLine()</code> should take care of enlarging the line
	 ...
	 */
	protected void addToken(Segment line, int tokenOffset, int tokenLength, byte id) {
		if (id >= Token.INTERNAL_FIRST && id <= Token.INTERNAL_LAST)
			throw new InternalError("Invalid id: " + id);

		if (tokenLength == 0 && id != Token.END)
			return;

		if (firstToken == null) {
			firstToken = new Token(line, tokenOffset, tokenLength, id);
			lastToken = firstToken;
		} else if (lastToken == null) {
			lastToken = firstToken;
			firstToken.line = line;
			firstToken.offset = tokenOffset;
			firstToken.length = tokenLength;
			firstToken.id = id;
		} else if (lastToken.next == null) {
			lastToken.next = new Token(line, tokenOffset, tokenLength, id);
			lastToken = lastToken.next;
		} else {
			lastToken = lastToken.next;
			lastToken.line = line;
			lastToken.offset = tokenOffset;
			lastToken.length = tokenLength;
			lastToken.id = id;
		}
	}
